...
public class AcaoRN {
	...
	public Acao carregar(String codigo, int x) {
	...
	public List<AcaoVirtual> listarAcaoVirtual(Usuario usuario)
		...
		try {
			...
			for (Acao acao : listaAcao) {
				...
				cotacao = this.retornaCotacao(
						YahooFinanceUtil.ULTIMO_PRECO_DIA_ACAO_INDICE, acao);
				if (cotacao != null) {
			...
			throw new RNException(
					"Não foi possível listar as ações. Por favor, tente novamente.");
		}
	...
	public String retornaCotacao(int indiceInformacao, Acao acao)
		...
		try {
			...
			System.err.println(ex.getMessage());
			throw new RNException("Não foi possível recuperar a cotação.");			
		}
		return informacao;
	}
