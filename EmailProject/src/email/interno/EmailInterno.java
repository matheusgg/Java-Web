...
public class EmailInterno {
	...
	private static final String SENHA_CONTA = "123";
	...
	private MultiPartEmail preparaEmail() {
		...
		try {
			...
			email.setDebug(true);
			email.setHostName(EmailInterno.SERVIDOR_SMTP);
			email.setSmtpPort(EmailInterno.PORTA_SMTP);
			email.setAuthenticator(new Autenticador(EmailInterno.CONTA_PADRAO,
					EmailInterno.SENHA_CONTA));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return email;
